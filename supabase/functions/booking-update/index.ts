...
serve(async (req) => {
  ...
  if (bk.tenant_id !== (prof as any).tenant_id) {
  ...
  try {
    await assertTenantActive(admin, (prof as any).tenant_id);
  } catch (e: any) {
    return withCors(
      req,
      JSON.stringify({
  ...
  if (typeof body?.session_id === "string" && body.session_id) {
    ...
    const { data: ns } = await admin.from("class_sessions").select(
      "id, capacity",
    ).eq("id", body.session_id).maybeSingle();
    if (ns?.capacity && ns.capacity > 0) {
      const { count } = await admin.from("bookings").select("*", {
        head: true,
        count: "exact",
      })
        .eq("tenant_id", (prof as any).tenant_id).eq(
          "session_id",
          body.session_id,
        )
        .in("status", ["booked", "checked_in"]).neq("id", id);
      if ((count ?? 0) >= ns.capacity) {
  ...
  const { data, error } = await admin.from("bookings").update(updates).eq(
    "id",
    id,
  ).select("*").single();
  if (error) {
