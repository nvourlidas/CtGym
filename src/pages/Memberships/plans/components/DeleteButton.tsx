...
export default function DeleteButton({ id, tenantId, onDeleted, guard }: {
  ...
  const handleConfirm = async () => {
    setBusy(true);
    const res = await supabase.functions.invoke('plan-delete', { body: { id, tenant_id: tenantId } });
    setBusy(false);
    setOpen(false);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border border-border/15 bg-secondary-background shadow-2xl p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-text-primary text-sm">Διαγραφή πλάνου</h3>
                <p className="text-xs text-text-secondary mt-0.5">Αυτή η ενέργεια δεν μπορεί να αναιρεθεί.</p>
              </div>
            </div>
            <p className="text-sm text-text-secondary mb-6">
              Είστε σίγουροι ότι θέλετε να διαγράψετε αυτό το πλάνο; Οι ενεργές συνδρομές που το χρησιμοποιούν ενδέχεται να επηρεαστούν.
            </p>
