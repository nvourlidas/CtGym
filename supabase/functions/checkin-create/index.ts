...
serve(async (req) => {
  ...
  const session_id = String(body?.session_id ?? ""), user_id = String(body?.user_id ?? "");
  if (!session_id || !user_id) return withCors(req, JSON.stringify({ error:"missing_fields" }), { status: 400 });
  ...
  const { data: ses, error: sErr } = await admin.from("class_sessions")
    .select("id, tenant_id, starts_at, ends_at").eq("id", session_id).maybeSingle();
  if (sErr || !ses) return withCors(req, JSON.stringify({ error: sErr?.message ?? "session_not_found" }), { status: 400 });
  if (ses.tenant_id !== tenantId) return withCors(req, JSON.stringify({ error:"tenant_mismatch" }), { status: 403 });
  ...
  const canTo   = end ? new Date(end.getTime() + 30 * 60 * 1000) : (start ? new Date(start.getTime() + 2 * 60 * 60 * 1000) : null);
  if ((canFrom && now < canFrom) || (canTo && now > canTo)) {
    return withCors(req, JSON.stringify({ error:"outside_checkin_window" }), { status: 409 });
  }

  // find or create booking
  let { data: bk } = await admin.from("bookings")
    .select("id, status").eq("tenant_id", tenantId).eq("session_id", session_id).eq("user_id", user_id)
    .order("created_at", { ascending:false }).limit(1);
  let bookingId = bk?.[0]?.id as string | undefined;

  if (!bookingId) {
    // walk-in: create a booking first (booked)
    const { data: created, error: cErr } = await admin.from("bookings")
      .insert({ tenant_id: tenantId, session_id, user_id, status:"booked" })
      .select("id").single();
    if (cErr) return withCors(req, JSON.stringify({ error: cErr.message }), { status: 400 });
  ...
  try {
    const { error: consumeErr } = await admin.rpc("consume_membership_session", {
      p_tenant_id: tenantId, p_user_id: user_id,
    });
  ...
  await admin.from("checkins").insert({ tenant_id: tenantId, session_id, user_id }).select().maybeSingle();
  ...
  if (uErr) return withCors(req, JSON.stringify({ error: uErr.message }), { status: 400 });

  return withCors(req, JSON.stringify({ ok:true, data: updated }), { status: 200 });
});
