...
serve(async (req) => {
  ...
  const { data: existing, error: exErr } = await admin.from("bookings").select("id, tenant_id").eq("id", id).maybeSingle();
  if (exErr || !existing) return withCors(JSON.stringify({ error: exErr?.message ?? "not_found" }), { status: 404 }, req);
