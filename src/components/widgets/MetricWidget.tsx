...
function parsePayload(query: string):
  ...
  | null {
  try { const obj = JSON.parse(query); return typeof obj === "object" && obj ? (obj as any) : null; }
  catch { return null; }
}
    ...
    const run = async () => {
      ...
      try {
        ...
        if (payload?.kind === "status_today" && payload.status) {
          ...
          const toISO = utcMidnightISO(new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1));
        ...
        if (payload?.kind === "revenue_cash_today" || payload?.kind === "revenue_cash_range") {
          ...
          if (payload.kind === "revenue_cash_today") {
            ...
            endISO = utcMidnightISO(new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1));
          } else {
          ...
          if (error) throw error;
          const total = (data as any[] | null)?.reduce((s, r) => s + Number(r.amount || 0), 0) ?? 0;
          setValue(total);
          return;
        }
        ...
        // …existing branches (raw SQL / time_series_count) stay as-is…
        if (isRawSQL) {
