...
async function sendExpoPush(params: {
  ...
  if (error) {
    return { ok: false, sent: 0, error: "token_query_failed", details: error.message };
  }
...
serve(async (req) => {
  ...
  const { isAdmin, tenant_id, user } = auth as { isAdmin: boolean; tenant_id: string | null; user: any };
  ...
  const adminMessage = typeof body?.message === "string" ? body.message.trim() : null;
  ...
  if (insErr) return withCors(JSON.stringify({ error: insErr.message }), { status: 400 }, req);

  // ✅ Predefined notification title/body
  const templateName = (tpl as any).name ?? "Πρόγραμμα";
  const notifTitle = "Νέο πρόγραμμα προπόνησης";
  let notifBody = `Σου ανατέθηκε το template: ${templateName}. Άνοιξε την εφαρμογή για να το δεις.`;
  if (adminMessage) {
