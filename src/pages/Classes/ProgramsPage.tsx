    ...
    <div className="p-6">
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          />
          <button
            className="h-9 rounded-md px-3 text-sm border border-white/10 hover:bg-white/5"
            onClick={generateRange}
            disabled={genBusy}
          >
            {genBusy ? 'Δημιουργία...' : 'Δημιουργία Συνεδριών'}
          </button>
        </div>
      </div>
      ...
      <div className="rounded-md border border-white/10 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading && (
              <tr><td className="px-3 py-4 opacity-60" colSpan={9}>Loading…</td></tr>
            )}
            {!loading && filtered.length === 0 && (
              <tr><td className="px-3 py-4 opacity-60" colSpan={9}>No rules</td></tr>
            )}
            {!loading && filtered.length > 0 && paginated.map(r => (
              <tr key={r.id} className="border-t border-white/10 hover:bg-secondary/10">
                ...
                <Td className="text-right">
                  ...
                  <button
                    className="ml-2 px-2 py-1 text-sm rounded hover:bg-secondary/10"
...
function DeleteRuleButton({ id, onDeleted }: { id: string; onDeleted: () => void }) {
  ...
  const onClick = async () => {
    if (!confirm('Διαγραφή αυτού του κανόνα; Αυτή η ενέργεια δεν μπορεί να αναιρεθεί.')) return;
    setBusy(true);
...
function RuleModal({
  ...
  return (
    <Modal onClose={onClose} title={mode === 'create' ? 'Νέος Κανόνας' : 'Επεξεργασία Κανώνα'}>
      <FormRow label="Τμήμα *">
      ...
      <FormRow label="Ημέρες">
        <WeekdayMulti value={weekdays} onChange={setWeekdays} />
      </FormRow>

      <div className="grid grid-cols-2 gap-3">
        <FormRow label="Επανάληψη κάθε (Εβδομάδες)">
          <input
            className="input"
            type="number"
            min={1}
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <FormRow label="Λήξη (προαιρετικό)">
          <input
            className="input"
            type="date"
            value={endsOn ?? ''}
      ...
      <div className="mt-4 flex justify-end gap-2">
        ...
        <button className="btn-primary" onClick={submit} disabled={busy}>
          {busy ? (mode === 'create' ? 'Δημιουργία…' : 'Αποθήκευση...') : (mode === 'create' ? 'Δημιουργία' : 'Αποθήκευση')}
        </button>
      </div>
    </Modal>
  );
...
function ExceptionsModal({ rule, onClose }: { rule: Rule; onClose: () => void }) {
  ...
  return (
    <Modal title={`Εξαιρέσεις — ${rule.starts_on} → ${rule.ends_on ?? '—'}`} onClose={onClose}>
      {err && (
        <div className="mb-3 text-sm border border-danger/30 bg-danger/10 text-danger rounded p-3">
          {err}
        </div>
      ...
      <div className="rounded-md border border-white/10 p-3 mb-4">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <FormRow label="Παράλειψη Ημερομηνίας">
            <label className="inline-flex items-center gap-2">
              <input
                type="checkbox"
                checked={skip}
          ...
          <FormRow label="Ανανέωση Χωρητικότητας">
            <input
              className="input"
              type="number"
              min={0}
              value={oCap}
              onChange={e => setOCap(e.target.value === '' ? '' : Number(e.target.value))}
            />
          </FormRow>
          <FormRow label="Ανανέωση ώρα έναρξης">
            <input
              className="input"
              type="time"
              value={oStart}
          ...
          <FormRow label="Ανανέωση ώρα λήξης">
            <input
              className="input"
              type="time"
              value={oEnd}
        ...
        <div className="mt-3 flex justify-end">
          <button className="btn-primary" onClick={submit} disabled={busy}>
            {busy ? 'Αποθήκευση...' : 'Αποθήκευση εξαίρεσης'}
          </button>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && rows.length === 0 && (
              <tr><td className="px-3 py-4 opacity-60" colSpan={7}>Καμία εξαίρεση</td></tr>
            )}
            {rows.map(x => (
              <tr key={x.id} className="border-t border-white/10 hover:bg-secondary/10">
                ...
                <Td className="text-right">
                  <button
                    className="px-2 py-1 text-sm rounded text-danger hover:bg-danger/10"
                    onClick={() => del(x.id)}
                  >
                    Διαγραφή
                  </button>
