...
export default function CreateWorkoutTemplateModal({ open, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    // load categories + equipment from your DB/api
    // Assumption: you have tables or views for these. Adjust queries if your schema differs.
    (async () => {
      try {
        const [catsRes, eqRes] = await Promise.all([
          supabase.from('wger_categories').select('id,name').order('name', { ascending: true }),
        ...
        console.error(e);
        // not fatal; you can still show empty state
      }
    })();
  }, [open]);
  ...
  const saveTemplate = async () => {
    ...
    try {
      ...
      // 1) create template workout row (edge function)
      ...
      const templateId = (res.data as any)?.data?.id as string;
