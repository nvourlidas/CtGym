...
serve(async (req) => {
  ...
  // Load booking first so we know the tenant to validate against
  ...
  // Caller must be admin/owner in the same tenant as the booking
  ...
  // Subscription gate
  try {
    await assertTenantActive(admin, String(existing.tenant_id));
  } catch (e: any) {
    return withCors(
      JSON.stringify({
  ...
  // Delete booking + restore session if needed
  ...
  return withCors(
    JSON.stringify({ ok: true, refundedRows }),
    { status: 200 },
    req,
  );
});